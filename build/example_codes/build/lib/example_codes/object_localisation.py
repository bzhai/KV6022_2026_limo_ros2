...
class Detector3D(Node):
    ...
    def __init__(self):    
        super().__init__('Detector_localisation_node')
        self.bridge = CvBridge()

        # subscribers and publishers
        ccamera_info_topic = '/limo_camera/camera_info_raw'
        dcamera_info_topic = '/limo_camera/depth/camera_info'
        cimage_topic = '/limo_camera/image'
        dimage_topic = '/limo_camera/depth/image_raw'
        self.camera_frame = 'depth_link' 

        self.ccamera_info_sub = self.create_subscription(CameraInfo, ccamera_info_topic, self.ccamera_info_callback, qos_profile=qos.qos_profile_sensor_data)
        self.dcamera_info_sub = self.create_subscription(CameraInfo, dcamera_info_topic, self.dcamera_info_callback, qos_profile=qos.qos_profile_sensor_data)
        self.cimage_sub = self.create_subscription(Image, cimage_topic, self.image_color_callback, qos_profile=qos.qos_profile_sensor_data)
        self.dimage_sub = self.create_subscription(Image, dimage_topic, self.image_depth_callback, qos_profile=qos.qos_profile_sensor_data)
